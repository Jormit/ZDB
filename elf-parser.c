...
void print_func_table64(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[], uint32_t symbol_table, uint32_t plt_table, Elf64_Addr plt_mem) {
	...
	for(i=0; i< symbol_count; i++) {
		...
		if (plt_table != 0 && plt_offset < plt_count && i > 0 && ((ELF64_R_SYM(plt_tbl[plt_offset].r_info) == i))){
...
char *search_func_tbl_by_addr(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[], 
	uint32_t symbol_table, uint32_t plt_table, Elf64_Addr plt_mem, long search_addr) {
...
char *search_funcs_by_addr(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[], long search_addr) {
	...
	for(i=0; i<eh.e_shnum; i++) {
		if (sh_table[i].sh_type==SHT_SYMTAB) {
			char* result = search_func_tbl_by_addr(fd, eh, sh_table, i, 0, 0, search_addr);
			if (result != NULL){
				return result;
			}
		} else if (sh_table[i].sh_type==SHT_DYNSYM) {
			char* result = search_func_tbl_by_addr(fd, eh, sh_table, i, plt_rela_table, plt_address, search_addr);
			if (result != NULL){
				return result;
			}
		}
...
struct search_term search_funcs64(int32_t fd, Elf64_Ehdr eh, Elf64_Shdr sh_table[], char *query) {
	uint32_t i;
	...
	for(i=0; i<eh.e_shnum; i++) {
		if (sh_table[i].sh_type==SHT_SYMTAB) {
			struct search_term result = search_func_table64(fd, eh, sh_table, i, query);
			if (result.size != 0){
				return result;
			}
		}
